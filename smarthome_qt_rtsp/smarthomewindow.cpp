    ...
    , ui(new Ui::SmartHomeWindow)
{
    ...
    /* 监控页 */
    monitor = new Monitor;
    monitor->setCursor(Qt::BlankCursor); // 隐藏鼠标指针
    connect(monitor,&Monitor::backto_main_page,this,&SmartHomeWindow::show_main_page_monitor);
...
/**
 * @brief SmartHomeWindow::show_main_page_monitor
 * 主页显示监控页隐藏
 */
void SmartHomeWindow::show_main_page_monitor()
{
    this->show();
    monitor->hide();
}
...
/**
 * @brief SmartHomeWindow::on_monitor_button_clicked
 * 主页隐藏监控页显示
 */
void SmartHomeWindow::on_monitor_button_clicked()
{
    this->hide();//主页隐藏,不释放窗口
    monitor->show();
}
